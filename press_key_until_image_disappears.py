...
def disappear_key(image_to_stop_on: str, key_to_press: str, interval: float, final_button_combo: str):
    while keep_pressing:
        actions.key(key_to_press)
        time.sleep(interval)
        if not actions.user.image_appeared_on_screen(image_to_stop_on):
            actions.user.stop_image_disappear_keypress(final_button_combo, False)
  
@mod.action_class
class Actions:
    def press_key_until_image_disappears(image_to_stop_on: str, key_to_press: str, interval: float, final_button_combo: str, stop_grinding: str = "True"):
        ...
        if keypress_thread is None or not keypress_thread.is_alive():
            keypress_thread = threading.Thread(target=disappear_key, args=(image_to_stop_on, key_to_press, interval, final_button_combo))
            keypress_thread.start()

    def stop_image_disappear_keypress(final_button_combo: str = "stop", interval: float = 0.3):
